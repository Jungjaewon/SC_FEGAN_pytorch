...
class Solver(object):
    ...
    def build_model(self):
        ...
        if self.lr_decay_policy == 'LambdaLR':
            self.g_scheduler = torch.optim.lr_scheduler.LambdaLR(self.g_optimizer, lr_lambda=lambda epoch: 0.99 ** epoch)
            self.d_scheduler = torch.optim.lr_scheduler.LambdaLR(self.d_optimizer, lr_lambda=lambda epoch: 0.99 ** epoch)
        ...
        for e in range(epoch_r, self.epoch):
            for i in range(iterations):
                ...
                if (i + 1) % self.g_critic == 0:
                    ...
                    g_loss_gt = self.l1_loss(mask * fake_image, mask * face_gt) * self.alpha
                    g_loss_gt += self.l1_loss((1 - mask) * fake_image, (1 - mask) * face_gt) * self.lambda_g_gt
                    g_loss += g_loss_gt
                    ...
                    g_loss += percep_loss + style_loss

                    if self.use_tv_loss:
                        B, C, H, W = comp_image.size()
                        g_loss_tv = self.lambda_g_tv * (torch.sum(torch.abs(comp_image[:, :, :, 1:] - comp_image[:, :, :, :-1])) +
                    ...
                    # Logging.
                    loss['G/fake_loss'] = self.lambda_g_fake * g_loss_fake.item()
                    loss['G/percep_loss'] = percep_loss
                    loss['G/style_loss'] = style_loss
                    loss['G/loss_gt'] = g_loss_gt.item()
                    if self.use_tv_loss:
